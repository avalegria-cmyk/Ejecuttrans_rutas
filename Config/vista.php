...
<aside class="sidebar" id="sidebar" aria-label="Navegación principal">
    ...
    <?php foreach(['dashboard'=>'Dashboard','recorridos'=>'Recorridos','buses'=>'Buses / discos','rutas'=>'Rutas','usuarios'=>'Usuarios'] as $key=>$label): if($key==='usuarios' && $u['rol']!=='admin') continue; ?>
        <a class="<?= $modulo===$key?'selected':'' ?>" <?= $modulo===$key?'aria-current="page"':'' ?> href="/Web/admin/<?= $key ?>.php"><?= iconoMenu($key) ?><span><?= e($label) ?></span></a>
    <?php endforeach ?>
        <a class="<?= $modulo==='perfil'?'selected':'' ?>" <?= $modulo==='perfil'?'aria-current="page"':'' ?> href="/App/conductor/perfil.php"><?= iconoMenu('perfil') ?><span>Mi perfil</span></a>
    </nav>
    <div class="sidebar-foot">
        ...
        <?php if($u['rol']==='admin'): ?><a class="sidebar-app" href="/App/conductor/dashboard.php"><?= iconoMenu('app') ?>Ir a la App</a><?php endif ?>
        <?php if($mostrarSalida): ?><a class="sidebar-logout" href="/auth/logout.php"><?= iconoMenu('salir') ?>Cerrar Sesión</a><?php endif ?>
    </div>
</aside>
<button class="sidebar-backdrop" id="sidebarBackdrop" type="button" aria-label="Cerrar menú" hidden></button>
<?php endif ?>
<div class="workspace"><header class="topbar"><div class="topbar-start"><?php if($u['rol']!=='conductor'): ?><button type="button" class="sidebar-toggle" id="sidebarToggle" aria-controls="sidebar" aria-expanded="true" aria-label="Ocultar menú"><span class="hamburger-lines" aria-hidden="true"><i></i><i></i><i></i></span></button><?php endif ?><span><?= $u['rol']==='conductor'?'EJECUTTRANS · CONDUCTOR':'Panel de operación' ?></span></div><div class="user-menu"><a class="user-link" href="/App/conductor/perfil.php"><?= e($u['nombres']) ?></a><span class="badge"><?= e($u['rol']) ?></span><?php if($mostrarSalida): ?><a class="logout-link" href="/auth/logout.php"><span aria-hidden="true">↪</span> Salir</a><?php endif ?></div></header><main class="content"><div class="page-heading"><div><h1><?= e($titulo) ?></h1></div><div class="page-heading-actions"><span class="date"><?= date('d/m/Y') ?></span><button type="button" class="export-excel" id="exportarExcel" data-modulo="<?= e($modulo) ?>" data-csrf="<?= e(csrf()) ?>"><?= iconoMenu('excel') ?><span>Exportar Excel</span></button><span id="estadoExportacion" class="export-status" role="status" aria-live="polite"></span></div></div>
<?php }
function pie(): void { ?></main><footer>Ejecuttrans · Registro de kilometraje</footer></div></div><script src="/Assets/js/common.js"></script><script src="/Assets/js/sidebar.js"></script><script src="/Assets/js/exportar_excel.js" defer></script></body></html><?php }
