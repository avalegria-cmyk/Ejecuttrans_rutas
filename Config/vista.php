...
<?php foreach(['dashboard'=>'Recorridos','buses'=>'Buses / discos','rutas'=>'Rutas','usuarios'=>'Usuarios'] as $key=>$label): if($key==='usuarios' && $u['rol']!=='admin') continue; ?><a class="<?= $modulo===$key?'selected':'' ?>" href="/Web/admin/<?= $key ?>.php"><?= e($label) ?></a><?php endforeach ?>
...
<?php endif ?>
<div class="workspace"><header class="topbar"><span><?= $u['rol']==='conductor'?'EJECUTTRANS · CONDUCTOR':'Panel de operación' ?></span><div class="user-menu"><a class="user-link" href="/App/conductor/perfil.php"><?= e($u['nombres']) ?></a><span class="badge"><?= e($u['rol']) ?></span><?php if($mostrarSalida): ?><a class="logout-link" href="/auth/logout.php"><span aria-hidden="true">↪</span> Salir</a><?php endif ?></div></header><main class="content"><div class="page-heading"><div><p class="eyebrow">CONTROL DE RECORRIDOS</p><h1><?= e($titulo) ?></h1></div><span class="date"><?= date('d/m/Y') ?></span></div>
<?php }
